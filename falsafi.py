...
class Core(Basis):
    def __init__(self, SchemaID, Lid, DmnID):
        super(Core, self).__init__(SchemaID, Lid, DmnID)
        self.name = ''
        self.usedforid = 0
        self.Group = []
        self.Cat = []
...
class schema(Core):
    def __init__(self, SchemaID, Lid, DmnID):
        super(schema, self).__init__(SchemaID, Lid, DmnID)
        result = {}
        _schema = [
            "field_name",
            "Hotel Name",
        ...
        for item in _schema:
            try:
                setattr(self, item, hotel_collection[item])
                result.update({item: hotel_collection[item]})
                # print(result)
            except:
                result.update({item: None})

        print('_schema length: ', len(_schema))
