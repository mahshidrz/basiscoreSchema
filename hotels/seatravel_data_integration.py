...
class SeatravelMappingProccess:
    ...
    def get_hotel_info(self):
        ...
        try:
            ...
            city = {"title": city["title"], "id": city["id"]}
        except:
            city = {"title": '', "id": ''}
        try:
            country_seatravel = hotel_list["informationn"]["country"]["id"]
            country = db.country.find_one({"ws.23": country_seatravel})
            country = {"title": country["title"], "id": country["id"]}
        except:
            country = {"title": "", "id": ""}
        self.giata = hotel_list["GiataId"]
        try:
            self.photo = ["http://service.seatravel.com.tr/media" + i['url'] for i in hotel_list["informationn"]["seasons"][0]["mediaFiles"]]
        except:
            self.photo = []
        return {"hotellist": hotel_list, "id": hotel_list["Id"], "city": city, "country": country}
    ...
    def add_mark_collection(self, id):
        db.test.update({"Id": id}, {'$set': {'mark': 1}})
    ...
    def finding_usedforid(self):
        ...
        if used_for_id == 0:
            ...
            method = 'insert'
        self.insert_image(int(used_for_id))
        return used_for_id, method

    def insert_image(self, used_for_id):
        if len(self.photo) != 0:
            photo_in_webservimage = list(db.webserviceimage.find({'used_for_id': used_for_id}))
            if len(photo_in_webservimage) == 0:
                db.webserviceimage.insert({"basis_id": used_for_id, "photo": self.photo})
            else:
                image_prev = photo_in_webservimage[0]['photo']
                image_prev.append(self.photo)
                image_prev = list(set(image_prev))
                db.webserviceimage.update({"basis_id": used_for_id}, {"$set": {"photo": image_prev}})
    ...
    def set_status(self):
        db.test.update({"GiataId": self.giata}, {"$unset": {"mark": ""}})
