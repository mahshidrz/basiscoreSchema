...
class VanilaMappingProccess:
    ...
    def set_facility_value(self, hotel, hotel_info):
        ...
        hotel_facility = [self.dictionary['schema_m1']['dict_hotel_facilities'][item] for item in hotel_facility if item != '0']

        room_facility = hotel['hoteldetail']['RoomFacilityIds']['int']
        room_facility = [self.dictionary['schema_m1']['dict_room_facilities'][item] for item in room_facility if item != '0']

        total_basis_facilities = {}
        for fixed_item in self.dictionary['schema_m1']['fixed_list']:
            for title, options in fixed_item.items():
                common_hotel_facility = list(set(hotel_facility).intersection(options))
                common_room_facility = list(set(room_facility).intersection(options))
        ...
        remaining = (set(room_facility) | set(hotel_facility)) - total_basis_facilities
        self.remaining_facility.extend(remaining)
        return hotel_info

    def set_field_value_to_basis_title(self, hotel):
        print(hotel['HotelId'])
        self.giata_code = int(self.get_giata_code(hotel['HotelId']))
        if self.giata_code:
            hotel_info = {}
